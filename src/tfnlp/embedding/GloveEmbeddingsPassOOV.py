...
class GloveEmbeddings(WordEmbeddings):
    ...
    def load_based_word2cnt(self,file_name,word2cnt,max_vocab_size = None):
        #print "call load_based_word2cnt"
        word2vec = {}
        for line in open(file_name):
            line = line.strip().split()
            try: 
        ...
        dim = len(list(word2vec.items())[0][1])
        sorted_word2cnt = sorted(list(word2cnt.items()),key = lambda x:x[1],reverse = True)
        for word,cnt in sorted_word2cnt:
            if word in word2vec:
                #self.embeddings.append(word2vec[word])
                self.embeddings.append(self.get_init_vector(dim))
                #print word.encode("utf-8"),True
            else:
                #print word.encode("utf-8"),False
                #continue
                self.embeddings.append(self.get_init_vector(dim))
            self.id2word[len(self.id2word)] = word
            self.word2id[word] = len(self.word2id)
...
if __name__ =="__main__":
    ...
    for i,word in list(embed.id2word.items()):
